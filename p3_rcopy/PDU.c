...
uint8_t *buildPDU(uint8_t* pdu, uint8_t* payload, uint32_t payloadLen, uint32_t sequenceNum, uint8_t flag){
    uint32_t seqNum_Net = htonl(sequenceNum);
    uint16_t checksum = 0;
    static uint8_t PDU[HEADER_LEN + MAX_PAYLOAD]; //handle - null + length field
